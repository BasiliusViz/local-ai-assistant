#!/usr/bin/env bash
# Развёртывание с нуля: поднимает стек, проверяет доступность модели,
# индексирует данные. Проверяет каждый шаг и останавливается на первой ошибке,
# объясняя, что делать.
#
#     ./deploy.sh
#
# Повторный запуск безопасен: контейнеры не пересоздаются зря, индексация
...
for model in "${GEN_MODEL}" "${EMBED_MODEL}"; do
    if echo "$tags" | grep -q "${model%%:*}"; then
        ...
        fail "на сервере ${OLLAMA_BASE} нет модели $model. Она должна быть там развёрнута"
    fi
done
