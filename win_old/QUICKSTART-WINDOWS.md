...
- **Сетевой доступ к Ollama** и её адрес. Проверить:
  ...
  значит их надо развернуть на том сервере — это делают те, кто его обслуживает
...
`JIRA_SINCE` — глубина истории: `30d`, `6M` или дата `2026-08-01`. Ставьте
сразу: почти всё время первого прогона уходит на эмбеддинги, и десять тысяч
задач считаются на порядок дольше тысячи, а задачи, которые не трогали год, в
поиске всё равно шумят. Разово окно перебивается ключом `--since 90d`.
Осторожно с буквами: строчная `m` в JQL — это **минуты**, месяцы — заглавная `M`.
...
**Модель не найдена на сервере.** Она должна быть развёрнута на той машине,
где живёт Ollama. Отсюда её добавить нельзя.
